...
**Network level characteristics. Find the overal network level of Density, Transitivity and Resciprocity:**
...
```
We can see note with ID 2 is the person with the largest Centrality degree ("in") when looking at people seeking him for advice. 
...
```
Using centrality betweensess we get that ID 18 is the most central Advicer.
...
```
Using centrality betweensess we get that ID 17 is the most central friend.
...
```
We can see that people from the same level (hirarchy) are a little more likely to become friends.
...
```
We see people who has the same tenuere are more likely to give each other advice

```{r}
##Friends
assortativity(g_f, V(g_f)$TENURE, directed = TRUE)
```
We see people who has the same tenuere are less likely to be friends with each other. 
...
We now want to find the percentage of mutual friends who also give each other advice: 
...
graphjs(g_f_cent, vertex.size = Size  , vertex.color= size_vector$LEVEL, bg= "pink")
```
